...
description: How apps/migrations owns the PostgreSQL schema as an independent Alembic app, the five-revision chain under apps/migrations/migrations/versions, and the schema-ownership rules across raw/core/analytics/ops.
...
| `ops` | Pipeline | `ops.pipeline_runs` writes via `SqlAlchemyPipelineRunRepository` | API does not currently expose `pipeline_runs`. |
