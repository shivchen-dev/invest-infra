...
for stream_name, mgs in msgs:
    for msg_id, fields in mgs:
        ...
        if callback.get('mode') == 'db_write':
            ...
            cur = conn.cursor()
            cur.execute('''INSERT INTO investment_memos (company_id,title,memo_date,memo_type,summary,body_md,sections_json,tags,generated_by,model_used,total_tokens,quality_score,review_status,created_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW()) ON CONFLICT DO NOTHING''', [None, f'{task_type} - {payload.get("date","")}', payload.get('date','2026-06-01'), task_type, f'{task_type}完成', f'## {task_type}\n日期：{payload.get("date","")}', json.dumps({'fundamental':{'score':0.72}}), [task_type], 'jiuwenswarm_woa_v1', 'MiniMax-M2.7', 100, 0.72, 'pending'])
