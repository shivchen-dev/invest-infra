"""Analytics-owned deterministic factor calculator (GOV-03).

Moved from :mod:`invest_domain.research.factor_calculators` to satisfy
``docs/ARCHITECTURE-GOVERNANCE.md`` §2 / §6: "Analytics owns factor
calculation and observations". The implementation is byte-for-byte the
same as the prior version, only the owning module changed. Research
observes the output through :class:`FactorCalculationResult` /
:class:`~invest_domain.research.models.FactorObservation` and never
re-derives the formulas.

Behavioural contract (unchanged):

* Pure function: same ``bars``, same ``as_of_date``, same
  ``instrument_id`` always produce the same
  :class:`FactorCalculationResult` in the same order.
* Quantum-rounded ``Decimal`` outputs (``0.00000001``,
  ``ROUND_HALF_EVEN``) so bytewise hashes remain stable across runs.
* Every v1.0.0 factor key from
  :data:`invest_domain.research.factor_set.FACTOR_DEFINITIONS` is
  produced exactly once; missing data surfaces as ``None`` with a
  matching :class:`~invest_domain.research.models.QualityStatus`.
* A companion ``MarketSnapshot`` and ``DataQuality`` block are emitted
  in the same call so Research can wire the result into an
  ``EvidencePack`` without consulting a second source.
"""
