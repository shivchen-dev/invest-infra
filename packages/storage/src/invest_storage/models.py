...
class StockPriceLimitRow(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("length(regime_id) > 0", name="regime_id_nonempty"),
        CheckConstraint("length(status) > 0", name="status_nonempty"),
        CheckConstraint(
            "length(source_provider) > 0",
            name="source_provider_nonempty",
        ),
        CheckConstraint("length(row_hash) = 64", name="row_hash_len64"),
