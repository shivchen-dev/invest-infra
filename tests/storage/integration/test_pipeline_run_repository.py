...
every change after the test, so the five tests in this file can share a
single container without leaking data between cases.

The fixtures create the ``ops.pipeline_runs`` schema (with the
six-value ``status`` vocabulary enforced by a CHECK constraint) directly
through ``Base.metadata.create_all``. The repository only relies on the
SQLAlchemy model so the migration introduced in ``20260731_0001`` is
exercised on a real PostgreSQL deployment rather than re-implemented in
the fixture.
"""
...
def test_start_and_complete_full_lifecycle(
    ...
    run = _new_run(job_key="etf_daily", started_at=started_at)
...
def test_mark_failed_records_error_summary(
    pipeline_run_repository: SqlAlchemyPipelineRunRepository,
) -> None:
    """``mark_failed`` stores the error summary and finishes the run."""
    ...
    fetched = pipeline_run_repository.get_by_id(started.id)
    assert fetched is not None
    assert fetched.error_summary == "provider timeout"
...
def test_concurrent_runs_have_distinct_ids(
    ...
    first = pipeline_run_repository.start(
        _new_run(job_key="etf_daily", started_at=base)
    )
...
def test_list_recent_filters_by_status(
    ...
    assert failed_recent[0].error_summary == "boom"

    assert pipeline_run_repository.count_by_status("running") == 2
    assert pipeline_run_repository.count_by_status("succeeded") == 1
    assert pipeline_run_repository.count_by_status("failed") == 1
    assert pipeline_run_repository.count_by_status("queued") == 0
...
def test_unknown_status_violates_check_constraint(
    ...
    The six-value vocabulary (``queued``, ``running``, ``succeeded``,
    ``failed``, ``partial``, ``cancelled``) is enforced at the database
    level by the ``ck_pipeline_runs_status_valid`` constraint introduced
    in migration ``20260731_0001``. The repository never bypasses that
    vocabulary itself, so we exercise the schema-level guard by running
    a raw ``UPDATE`` through the same session.
    """
    ...
    with pytest.raises(IntegrityError):
        db_session.execute(
            text(
                'UPDATE "ops"."pipeline_runs" SET status = \'mystery\' WHERE id = :id'
            ),
            {"id": started.id},
        )
