"""Eastmoney (东方财富) adapter configuration (three-provider plan, Phase 1).

The settings object mirrors the
:class:`~invest_pipeline.adapters.cifang.config.CifangSettings` shape
because Eastmoney shares the documented "no adjustment" contract (the
public historical-quotes endpoint treats an empty ``adjust`` value as
"no adjustment") and the source is a public, non-official endpoint with
no documented API key.

Design rules:

- ``enabled`` defaults to ``False``; the adapter is opt-in only (matrix
  §6, ``INVEST_PIPELINE_EASTMONEY_ENABLED`` env var).
...
class EastmoneySettings(BaseSettings):
    """Redacted, disabled-by-default configuration for the Eastmoney adapter.
    ...
    The public Eastmoney endpoint does not require credentials, so the
    settings expose **no** credential field. The ``adjustment`` lock
    is enforced here (rather than as a Pydantic field validator) so
    the rejection message is explicit and the value cannot be silently
    coerced.
    """

    model_config = SettingsConfigDict(
        env_prefix="INVEST_PIPELINE_EASTMONEY_",
        extra="ignore",
    )
    ...
    def model_post_init(self, __context: object) -> None:
        ...
        ``adjustment="none"`` rule. Eastmoney uses ``"none"`` as the
        ...
        if self.adjustment != _ADJUSTMENT_NONE:
            raise ValueError(
                "EastmoneySettings.adjustment must be 'none' "
                "(ADR-0005 §4 / three-provider plan §Risks); "
                f"got {self.adjustment!r}"
            )
        if self.timeout_seconds <= 0:
            raise ValueError(
                f"EastmoneySettings.timeout_seconds must be > 0; got {self.timeout_seconds!r}"
            )
