"""Sina (新浪) adapter configuration (three-provider plan, Phase 1).

The settings object mirrors the Eastmoney
:class:`~invest_pipeline.adapters.eastmoney.config.EastmoneySettings`
shape because Sina shares the documented "no adjustment" contract on
its public historical-quotes endpoint and the source is a public,
non-official endpoint with no documented API key.

Design rules:

- ``enabled`` defaults to ``False``; the adapter is opt-in only (matrix
  §6, ``INVEST_PIPELINE_SINA_ENABLED`` env var).
...
class SinaSettings(BaseSettings):
    """Redacted, disabled-by-default configuration for the Sina adapter.
    ...
    The public Sina endpoint does not require credentials, so the
    ...
    model_config = SettingsConfigDict(
        env_prefix="INVEST_PIPELINE_SINA_",
        extra="ignore",
    )
    ...
    def model_post_init(self, __context: object) -> None:
        ...
        ``adjustment="none"`` rule. Sina uses ``"none"`` as the
        ...
        if self.adjustment != _ADJUSTMENT_NONE:
            raise ValueError(
                "SinaSettings.adjustment must be 'none' "
                "(ADR-0005 §4 / three-provider plan §Risks); "
                f"got {self.adjustment!r}"
            )
        if self.timeout_seconds <= 0:
            raise ValueError(
                f"SinaSettings.timeout_seconds must be > 0; got {self.timeout_seconds!r}"
            )
