    ...
    def _fetch(self, request: ProviderRequest, fetch: Callable, mapper: Callable):
        ...
        if not self._settings.enabled:
            raise RealProviderRequiresExplicitEnablementError(
                "tushare provider requires TushareSettings.enabled=True (INVEST_PIPELINE_TUSHARE_ENABLED)"
            )
        try:
            response = fetch()
            batch_id = uuid4()
            ...
            digest = sha256(
                json.dumps(
                    response.raw_payload, ensure_ascii=False, sort_keys=True, separators=(",", ":")
                ).encode()
            ).hexdigest()
            return (
                request,
