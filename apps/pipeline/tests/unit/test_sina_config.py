"""Unit tests for :class:`invest_pipeline.adapters.sina.config.SinaSettings`.

Phase 1 of the V2 three-provider plan
(``tasks/plan-data-source-three-provider.md``) freezes the Sina adapter
configuration contract. The settings object is intentionally narrow —
it captures only the documented safety rules in ADR-0005 §4 and the
plan §"Risks and Mitigations" table. The tests assert:
...
from invest_pipeline.adapters.sina.config import SinaSettings


class SinaSettingsDefaultsTest(unittest.TestCase):
...
class SinaSettingsAdjustmentLockTest(unittest.TestCase):
    ...
    def test_hfq_adjustment_is_rejected(self) -> None:
        ...
        with self.assertRaises(ValueError):
            SinaSettings(adjustment="hfq")

    def test_empty_adjustment_is_rejected(self) -> None:
        # The Sina "no adjustment" literal is the string ``"none"``.
        # The empty string (AkShare's "no adjustment" literal) must
        # not be silently coerced.
        with self.assertRaises(ValueError):
            SinaSettings(adjustment="")


class SinaSettingsTimeoutTest(unittest.TestCase):
    """``timeout_seconds`` must be a positive float."""

    def test_zero_timeout_is_rejected(self) -> None:
        with self.assertRaises(ValueError) as ctx:
            SinaSettings(timeout_seconds=0)
        self.assertIn("timeout_seconds", str(ctx.exception))

    def test_negative_timeout_is_rejected(self) -> None:
        with self.assertRaises(ValueError):
            SinaSettings(timeout_seconds=-1.0)


class SinaSettingsRedactionTest(unittest.TestCase):
    ...
    def test_redacted_dict_exposes_every_field(self) -> None:
        ...
        view = SinaSettings().redacted_dict()
    ...
    def test_redacted_dict_reflects_overrides(self) -> None:
        ...
        settings = SinaSettings(enabled=True)
        view = settings.redacted_dict()
        self.assertEqual(view["enabled"], "True")


class SinaSettingsImportSafetyTest(unittest.TestCase):
    ...
    def test_importing_config_does_not_import_httpx(self) -> None:
        ...
        import invest_pipeline.adapters.sina.config as config_module
