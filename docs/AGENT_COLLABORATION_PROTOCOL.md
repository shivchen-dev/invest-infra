...
version: 1.6
last_updated: 2026-06-13T19:11:40+08:00
author: RAA
approver: <user-2026-06-13-19:11>  # 用户批准 (QQ sender_id: DC04E397170345249B2A928500EEA2DF, message_id: ROBOT1.0_oKw0FxiTNqg3...)
status: approved
approved_at: 2026-06-13T19:11:40+08:00

supersedes: 1.5
trigger_source: 用户 18:56 (v1.5 移除 §12) + 用户 19:04 (v1.6 彻底移除 RAA 内部原则) + 用户 19:08 (按协议宗旨彻底修订)
---

# 投研系统智能体合作开发协议 v1.6

> **协议 ID**：`AGENT_COLLABORATION_PROTOCOL_v1.6`  
> **状态**：✅ **approved**（2026-06-13 19:11 CST，用户批准）  
...
> **v1.0 → v1.1 → v1.2 → v1.3 → v1.4 变更**：见附录 D / E / F / G / H / I
...
> **v1.6 修订**：原 §5.2 BOUNDARY OVERRIDE 协议 + §5.3 越界检测已移除（RAA 内部纪律，移入 LEARNINGS.md）。
...
- **v1.5**（2026-06-13 18:57，draft）：基于 v1.4 接受用户 18:56 明确请求：移除 §12 RAA 报告精简原则（**RAA 内部原则，不属于投研系统协议**），§12 内容移入 LEARNINGS.md
- **v1.6**（2026-06-13 19:11，approved）：基于 v1.5 接受用户 19:04 + 19:08 反馈：协议宗旨 = 避免版本混乱；v1.5 自身有"RAA 内部原则混入"问题（§5.2 BOUNDARY OVERRIDE + §5.3 越界检测）。**彻底移除 RAA 内部原则到 LEARNINGS.md**，投研系统协议**仅**描述跨 Agent 协作
...
## 附录 H：v1.4 → v1.5 变更摘要（6/13 18:57 用户 18:56 触发）

> 本附录列出 v1.5 相对 v1.4 的**所有**变更。

### 触发源

- **2026-06-13 18:48 用户原话**：「这条是针对你的，不是针对投研系统的，不需要加入投研系统的协议」
- **2026-06-13 18:53 用户问**：「你看下投研系统协议条款有没有类似把你的内部原则加入到投研系统协作条款里去」
- **2026-06-13 18:56 用户回**「A」—— 接受建议：移除 §12 RAA 报告精简原则

### 1 项核心变更

#### 移除 §12 RAA 报告精简原则（v1.4 → v1.5）

**变更**：v1.4 §12 整章"12.1 报告核心 / 12.2 路径报告原则 / 12.3 精简版 vs 完整版"**全部移除**。

**理由**：
- §12 是 RAA **单方面**工作规范（"RAA 报告格式"）
- 其他 Agent（Arc/CIA）**不用** RAA 报告格式
- §12 **不**是"跨 Agent 协作原则"，**不**应写入投研系统协议
- **RAA 内部原则** ≠ **投研系统协议条款**

**§12 内容去向**：**LEARNINGS.md**（RAA 内部档案，v1.4 §6.3.4 违规后果要求"写入 LEARNINGS.md 作为教训"，本条**同样适用**"RAA 内部工作规范 = LEARNINGS.md 范畴"）

### v1.5 未解决项（v1.6 候选）

- §5.2 "不推诿、不以'等指令'挡事" 边界（**协议** vs **RAA 内部原则**？v1.4 标"针对 RAA 要求"，但位置在协议里）
- §5.3 越界检测"RAA 必做扫描"（灰色地带 — 协议**应**规定"Re-Audit 必查项"，**不**应规定"RAA 必做扫描"）
- 软链接 `raa-{handoff,audit}-readonly` 自身 untracked（v1.2 §2.5 跨周期违反）
- v1.3 协议本身仍含 RAA 工作区路径（v1.3 已 approved 不重写，但 v1.3 supersedes by v1.4 后失效）

### 教训（LEARNINGS.md L-2026-06-13-04 重申）

> "RAA 内部原则 ≠ 投研系统协议条款。这条是 RAA 工作纪律，**不**写入 v1.4 / v1.5 / 任何投研系统协议。"

---

---

## 附录 I：v1.5 → v1.6 变更摘要（6/13 19:08 用户 19:04 + 19:08 触发）

> 本附录列出 v1.6 相对 v1.5 的**所有**变更。

### 触发源

- **2026-06-13 19:04 用户原话**：「撤销协议也是针对你的，不应该合作协议里」
- **2026-06-13 19:07 用户问**：「合作协议宗旨是什么，你知道吗」
- **2026-06-13 19:08 用户答**「A」—— 接受建议：v1.5 → v1.6 彻底移除 RAA 内部原则

### 关键自审

> 协议宗旨（用户 15:01 原话）= **避免版本混乱**
> 
> v1.5 协议**自身**有版本混乱：
> - 跨 Agent 协作（写权限、handoff、修复流程）✅ 正确
> - RAA 内部纪律（BOUNDARY OVERRIDE、越界检测、报告格式）❌ **混入** = 协议自身有版本混乱

### 移除项（v1.5 → v1.6 全部移到 LEARNINGS.md）

| 原 v1.5 条款 | 性质 | 去向 |
|---|---|---|
| **§5.2 BOUNDARY OVERRIDE 协议** | RAA 内部纪律 | LEARNINGS.md |
| **§5.3 越界检测 "RAA 必做扫描"** | RAA 内部操作 | LEARNINGS.md |
| §12 RAA 报告精简原则（v1.5 已移）| RAA 内部纪律 | LEARNINGS.md（v1.5 已做）|

### 协议 §5 标题变更

| v1.5 | v1.6 |
|---|---|
| ## 5. 边界铁律 | ## 5. Agent 写权限与冲突预防 |

> §5 标题从"RAA 边界视角"改为"Agent 写权限视角"，反映"协议**仅**约束跨 Agent 协作"。

### v1.6 协议**真正**应描述的（跨 Agent 协作）

- §1 Agent 角色 + 边界（写权限矩阵）
- §2 Git 协作
- §3 状态文件协议
- §4 修复流程
- §5 **Agent 写权限与冲突预防**（仅 §5.1 写权限矩阵回顾）
- §6 版本管理最佳实践（Git 流程）
- §7 通信协议
- §8 冲突解决
- §9 监督与审计
...
- v1.3 协议本身仍含 RAA 工作区路径（v1.3 已 approved 不重写，v1.3 supersedes by v1.4→v1.5→v1.6 后失效）
- 软链接 `raa-{handoff,audit}-readonly` 自身 untracked（v1.2 §2.5 跨周期违反）
- WOA / system-architect / data-architect / tech-expert 写权限（v1.4 时代未涉及）

### 教训（LEARNINGS.md L-2026-06-13-04 + L-2026-06-13-05 重申）

> "RAA 内部原则 ≠ 投研系统协议条款。这条是 RAA 工作纪律，**不**写入任何投研系统协议。"

---

*协议修订：RAA · 2026-06-13 14:58 CST v1.0 → 15:04 CST v1.1 → 15:24 CST v1.2 (approved) → 16:20 CST v1.3 (approved) → 18:27 CST v1.4 (approved) → 18:57 CST v1.5 (approved) → 19:11 CST v1.6 (approved)*  
*权威源：`invest-infra/raa-handoff-readonly/raa-handoff-protocol-v1.6-source.md`（RAA 工作区交接目录，**投研系统软链接可见**）*
*v1.0 已被 v1.1 取代（supersedes）；v1.1 已被 v1.2 取代（supersedes）；v1.2 已被 v1.3 取代（supersedes）；v1.3 已被 v1.4 取代（supersedes）；v1.4 已被 v1.5 取代（supersedes）*
