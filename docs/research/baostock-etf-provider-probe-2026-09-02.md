...
## 8. P0 真实验收补充（2026-09-02）

在不修改产品代码、不写数据库、不部署的前提下，对最新完整交易日
`2026-09-01` 完成三轮连续调用：16/16 标的每轮均返回 1 行，所有标的的
行级 `rows_sha256` 三轮完全一致。整份报告文件哈希因运行时间和耗时字段变化而不同，
不代表行情内容漂移。

跨源对账选取 `510300`。AkShare Adapter 通过其新浪行情路径返回的数据与 BaoStock
逐值一致：open `4.683`、high `4.705`、low `4.666`、close `4.684`、
volume `846798312`、amount `3962899774`。AkShare 东财直连路径仍出现
`RemoteDisconnected`，但 Adapter 的首选新浪路径可用。

在北京时间约 14:37 对尚未收盘的 `2026-09-02` 查询时，16/16 标的均为零行。
该结果仅说明 BaoStock 不提供可用于日线落库的盘中未收盘行，不计为异常，也不作为
收盘后当日增量通过证据。错误分类相关 BaoStock 单元回归共 75 项通过。

| 补充验收项 | 结果 |
|---|---|
| 连续调用稳定性 | 通过（3轮，16/16） |
| 最新完整交易日增量 | 通过（2026-09-01） |
| 行级内容哈希稳定性 | 通过 |
| 跨源数值一致性 | 通过（510300，BaoStock 对 AkShare/Sina） |
| 错误分类回归 | 通过（75项） |
| 未收盘当日日线 | 零行，符合保守落库要求 |

准入结论保持 `conditional_admit`：允许进入“仅近期 ETF 日线、仅瞬态故障触发”的
备用源编排设计与实现；禁止用于多年历史重建，禁止作为全局 Provider 替换 AkShare。
