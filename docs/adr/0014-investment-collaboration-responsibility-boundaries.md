...
```text
CIA          = 投研方向、策略目标、正式审批与投资判断
ARC          = 投研系统架构、开发、集成与技术验证
WorkBuddy    = 策略研发辅助、候选发现、研究、反证与观察执行
...
### 3.3 WorkBuddy：外部投研执行团队

WorkBuddy 负责：

- 利用已安装 Skills、金融 MCP、Connector、通达信和公开信息获取多源数据；
- 执行策略制定、数据能力盘点和策略变更提案任务；
- 按正式 StrategyVersion 执行候选发现；
- 对已准入候选开展深度研究、风险反证和证据交叉验证；
- 执行长期观察、事件复评和投资假设检查；
- 生成结构化交付物与人类可读报告；
- 基于多次正式运行结果提出 `StrategyChangeProposal`。
...
| StrategyProposal | invest-infra | WorkBuddy/CIA | RAA |
...
| 候选发现结果 | invest-infra 的 ExternalObservation | WorkBuddy | RAA抽查 |
...
```text
CIA确定方向和策略目标
→ invest-infra发布strategy任务
→ WorkBuddy提交StrategyProposal与验证材料
...
```text
invest-infra按active StrategyVersion发布candidate任务
→ WorkBuddy多源选股
→ invest-infra摄取ExternalObservation并正式准入
...
该分工使 OpenClaw 团队承担治理层：CIA 管方向和审批，ARC 管建设，RAA 管独立审计，Ops 管运行；WorkBuddy 作为外部投研执行团队，充分使用金融 MCP 和技能执行选股、研究与观察；invest-infra 保持数据、策略、结果、组合和审计记录的唯一权威。
