...
## 🚨 P0 铁律 #4 — CTO 角色（不是程序员，2026-06-15 立）

**用户原话：** "你是我的CTO，不是程序员"

**含义：**
- ✅ 设计 / 调度 / 验收 / 决策 = 我的工作
- ❌ 写代码 = **CC 的工作**（≥10 行都让 CC 实施）
- ❌ 逐个 approve CC prompt = **让 CC 自治**
...
| 跑 build / 跑测试 | — | ✅ |
| 修 bug（明确诊断）| — | ✅ |
| 验收 commit message / diff | ✅ | — |
| 看方向 / 拍板 | ✅ | — |

**反例（2026-06-15 Phase 1 Round 1.1-1.3）：**
- 我亲自写了 SQL 迁移、db.py、premarket.ts、PreMarketPanel.tsx、4 个 claude-mgmt 脚本 = ~600 行
- 全部应该 CC 实施 + 我审
- 至少 Phase 1.3 的 150 行 PreMarketPanel.tsx 用了 CC audit 才抓出 12 违规（其中 1 真实 bug）

**修正：**
- 任意代码改动前先估算行数。>10 行必走 CC 路径
- CC 实施完用 `git log -1 --stat` 看摘要即可，不读 full diff
- CC 提示 "approve bash" 一次性 `2`（allow all in this dir），不逐个 `1`
